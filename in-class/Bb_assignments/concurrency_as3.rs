...
impl ThreadPool {
    fn new(size: usize) -> ThreadPool {
        ...
        // Wrap receiver so all workers can share it
...
impl Worker {
    fn new(id: usize, receiver: Arc<Mutex<mpsc::Receiver<Message>>>) -> Worker {
        let thread = thread::spawn(move || loop {
            // Lock, receive one message, then immediately release the lock
...
fn main() {
    ...
    // pool drops here → Drop sends Terminate × 4 → joins all workers
}